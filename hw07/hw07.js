...
let shading = '(PHONG)';     // (PHONG) or (GOURAUD)
...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            cone.updateNormals();
            shadingMode = 'SMOOTH';
            updateText(textOverlay3, "shading mode: " + shadingMode + " " + shading);
            render();
        }
        else if (event.key == 'f') {
...
async function main() {
    try {
        ...
        textOverlay3 = setupText(canvas, "shading mode: " + shadingMode + " " + shading, 3);
